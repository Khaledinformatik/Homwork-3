...
class Client:
    def __init__(self, username: str, pw_bytes: bytes):
        ...
        self.pw = pw_bytes
        self.a = random_scalar()
        self.A = self.a * G

    def register_interactive(self, server: Server):
        server.create_account(self.username)
    ...
    def login(self, server: Server, salt: Optional[bytes] = None, transcript_info: Optional[bytes] = None) -> bool:
        ...
        x = random_scalar()
        X = x * G
        Y, SK_server = server.ake_3dh_server(self.username, X, lpkc, K_oprf, salt=salt, transcript_info=transcript_info)

        #  self.a
        SK_client_3dh = KClient_3DH(sk_c, x, server.db[self.username]["B"], Y)
